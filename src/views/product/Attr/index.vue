          ...
          <el-table style="width: 100%;margin: 20px 0" border :data="attrInfo.attrValueList">
            ...
            <el-table-column label="属性值名称" width="width">
              <template slot-scope="{ row, $index }">
                ...
                <el-input :ref="$index" @keyup.native.enter="toLook(row)" @blur="toLook(row)" v-if="row.flag" v-model="row.valueName" placeholder="请输入属性值名称" size="mini"></el-input>
                <span @click="toEdit(row, $index)" v-else style="display: block">{{ row.valueName }}</span>
              </template>
            </el-table-column>
            <el-table-column rop="prop" label="操作" width="width">
              <template slot-scope="{ row, $index }">
                <!-- 气泡确认框 -->
